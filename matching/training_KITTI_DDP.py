...
def train(model, optimizer, sample, loss_fn, exp_cfg, device, mode='pairs'):
    if True:
        ...
        batch_dict = model(model_in, delta_pose, metric_head, compute_embeddings,
...
def get_database_embs(model, sample, exp_cfg, device):
    ...
    with torch.no_grad():
        ...
        batch_dict = model(model_in, None, metric_head=False)
        anchor_out = batch_dict['out_embedding']
...
def main_process(gpu, exp_cfg, common_seed, world_size, args):
    ...
    if args.wandb and rank == 0:
        wandb.init(project="deep_NFC", name=dt_string, config=exp_cfg)
    ...
    for epoch in range(starting_epoch, exp_cfg['epochs'] + 1):
        ...
        for batch_idx, sample in enumerate(TrainLoader):
            #print(sample['positive_idx'])
            #print(sample['negative_idx'])
        ...
        if rank == 0:
            ...
            if (epoch % 5) == 0:
                ...
                torch.save(best_model, savefile20)
